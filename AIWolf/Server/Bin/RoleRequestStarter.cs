    ...
    class RoleRequestStarter
    {
        static readonly string description =
            "このクラスを利用して，自作Playerごとに役職を決めてゲームをスタートできます．\n" +
             "Usage:" + typeof(RoleRequestStarter) + " -n agentNum -c playerClass role [-c playerClass role] [-d defaultPlayer]\n" +
            ...
            typeof(RoleRequestStarter) + " -n 12 -c org.aiwolf.MyPlayer bodyguard\n" +
            "としてください．\n";

        public static void Main(string[] args)
        {
            List<KeyValuePair<string, Role?>> playerRoleList = new List<KeyValuePair<string, Role?>>();
            string defaultClsName = "AIWolf.TestPlayer";
            int playerNum = -1;
            string logDir = "./log/";
            ...
            if (playerNum < 0)
            {
                Console.Error.WriteLine("Usage:" + typeof(RoleRequestStarter) + " -n agentNum -c playerClass role [-c playerClass role...] [-d defaultPlayer] [-l logDir]");
                return;
            }
            ...
            {
                playerMap.Add((IPlayer)Activator.CreateInstance(Type.GetType(pair.Key)), pair.Value);
            }
